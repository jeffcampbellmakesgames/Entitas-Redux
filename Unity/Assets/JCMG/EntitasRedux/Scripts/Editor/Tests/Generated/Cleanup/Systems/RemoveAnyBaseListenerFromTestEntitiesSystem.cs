//------------------------------------------------------------------------------
// <auto-generated>
//		This code was generated by a tool (Genesis v1.2.1, branch:develop).
//
//
//		Changes to this file may cause incorrect behavior and will be lost if
//		the code is regenerated.
// </auto-generated>
//------------------------------------------------------------------------------
using System.Collections.Generic;
using JCMG.EntitasRedux;

public sealed class RemoveAnyBaseListenerFromTestEntitiesSystem : ICleanupSystem
{
	private readonly IGroup<TestEntity> _group;
	private readonly List<TestEntity> _entities;

	public RemoveAnyBaseListenerFromTestEntitiesSystem(IContext<TestEntity> context)
	{
		_group = context.GetGroup(TestMatcher.AnyBaseListener);
		_entities = new List<TestEntity>();
	}

	/// <summary>
	/// Performs cleanup logic after other systems have executed.
	/// </summary>
	public void Cleanup()
	{
		_group.GetEntities(_entities);
		for (var i = 0; i < _entities.Count; ++i)
		{
			_entities[i].RemoveAnyBaseListener();
		}
	}
}
